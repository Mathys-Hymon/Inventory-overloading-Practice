...
public:
    ...
    void Cook(const std::vector<IBakable*> ingredients) 
    {
		...
		for (const auto& recipe : _recipes) {
			...
			if (match) {
                std::cout << "\n\nYou have cooked and eaten " + recipe._output + " and it gives you " + EffectType::getEffectName(recipe._effect) + " for " + std::to_string((int)recipe._effectDuration) + " secondes\n\n";
				recipeFound = true;
                break;
			}
		}
